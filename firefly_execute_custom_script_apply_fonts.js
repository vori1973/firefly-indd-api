...
async function executeCustomScript({ scriptUrl, inddKey, fontPrefix, linksPrefix,label,font, style, outputPath, jobFolder }) {
  ...
  const fontAssets = await generateAssetEntriesFromS3Prefix(fontPrefix, ['ttf', 'otf', 'lst'], 'MyFonts');
...
(async () => {
  try {
    ...
    const label =  "MainPara";
    const font = "Super Shiny";
    const style =  "Regular";
   const pdfPreset = "High Quality Print";
